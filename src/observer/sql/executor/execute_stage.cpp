...
RC ExecuteStage::check_select(const Selects selects, const char *db){
  ...
  Db *datebae = dh->find_db(db);

  // select ** from ((check)) where **
  for (size_t i = 0; i < selects.relation_num; i++){
    if (nullptr == datebae->find_table(selects.relations[i])) return RC::INVALID_ARGUMENT;
  }

  // select ((check)) from ((check)) where **
  for (size_t i = 0; i < selects.attr_num; i++){
    RelAttr tmp_rel = selects.attributes[i];
    RC rc = check_attri_valid(tmp_rel, selects, datebae);
    //LOG_INFO("check_ nullptr%d ", rc);
    if (rc != SUCCESS) return RC::INVALID_ARGUMENT;
  }

  // select ** from ((check)) where ((check))
  for (size_t i = 0; i < selects.condition_num ; i++){
    Condition condition = selects.conditions[i];
    RC rc = check_condition_valid(condition, selects, datebae);
    if (rc != SUCCESS) return RC::INVALID_ARGUMENT;
  }
  return RC::SUCCESS;
}
