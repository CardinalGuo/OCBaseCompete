...
void get_max_min_value(std::vector<void *> &left, AttrType attr_left, std::vector<void *> &values_vec, int max_min)
{
    ...
    case INTS:
    {
        ...
        {
            int sub_v = *(int *)value - *(int *)left[i];
    ...
    case FLOATS:
    {
        ...
        memcpy(value, left[0], sizeof(float));
        for (size_t i = 1; i < left.size(); i++)
        {
            float sub_v = *(float *)value - *(float *)left[i];
            if (sub_v * max_min < 0)
            {
                memcpy(value, left[i], sizeof(float));
            }
        }
    }
    break;
...
void SelectExe::get_expression_name(std::string &str_name, Expression *expression, int deep)
{
    ...
    if (expression->is_leaf)
    {
        if (expression->is_attr)
        {
            ...
            {
                if (select->relation_name == 1 && select->join_num_max == 0)
                    str_name.append(expression->attr.relation_name);
                else 
                    str_name.append(name);
            }
        }
