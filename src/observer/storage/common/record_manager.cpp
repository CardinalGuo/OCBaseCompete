...
int page_fix_size()
{
  return sizeof(PageHeader::page_type) + sizeof(PageHeader::record_num) + sizeof(PageHeader::record_capacity) + sizeof(PageHeader::record_real_size) + sizeof(PageHeader::record_size) + sizeof(PageHeader::first_record_offset);
}
...
RC RecordPageHandler::init_empty_page(DiskBufferPool &buffer_pool, int file_id, PageNum page_num, int record_size, int page_type)
{
  ...
  page_header_->page_type = page_type;
...
bool RecordPageHandler::is_text() const
{
  return page_header_->page_type == 1;
}
RC RecordFileHandler::insert_record(const char *data, int record_size, RID *rid)
{
  ...
  for (int i = 0; i < page_count; i++)
  {
    ...
    if (!record_page_handler_.is_full() && !record_page_handler_.is_text())
    {
      page_found = true;
      break;
    }
  ...
  if (!page_found)
  {
    ...
    record_page_handler_.deinit();
    ret = record_page_handler_.init_empty_page(*disk_buffer_pool_, file_id_, current_page_num, record_size, 0);
    if (ret != RC::SUCCESS)
    {
...
RC RecordFileScanner::get_next_record(Record *rec)
{
  ...
  while (current_record.rid.page_num < page_count)
  {
    ...
    if (current_record.rid.page_num != record_page_handler_.get_page_num())
    {
      ...
      if (RC::BUFFERPOOL_INVALID_PAGE_NUM == ret || record_page_handler_.is_text())
      {
    ...
    else if (RC::RECORD_EOF == ret || record_page_handler_.is_text())
    {
      current_record.rid.page_num++;
      current_record.rid.slot_num = -1;
    }
