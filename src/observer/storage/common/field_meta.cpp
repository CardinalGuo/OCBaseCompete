...
RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int nullable)
{
  ...
  visible_ = visible;
  nullable_ = nullable;
  LOG_INFO("Init a field with name=%s type=%d len=%d, offset=%d", name, attr_type, attr_len, attr_offset);
  return RC::SUCCESS;
}
...
void FieldMeta::to_json(Json::Value &json_value) const
{
  ...
  //LOG_INFO("%d %s",attr_type_,attr_type_to_string(attr_type_));
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  {
    LOG_ERROR("Len is not an integer. json value=%s", nullable_value.toStyledString().c_str());
    return RC::GENERIC_ERROR;
  }
