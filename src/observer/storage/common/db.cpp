...
RC Db::drop_table(const char* table_name){
  ...
  auto i = opened_tables_.find(table_name);
  if (i == opened_tables_.end()) {
    return RC::SCHEMA_TABLE_NOT_EXIST;
  }
  i->second->~Table();

  const char* bd = i->second->bd().c_str();

  char* data_file_name = new char[50];
  memset(data_file_name, 0, 50);
  strcpy(data_file_name, bd);
  strcat(data_file_name, "/");
  strcat(data_file_name, table_name);
  strcat(data_file_name, ".data");
  remove(data_file_name);
  
  char* meta_file_name = new char[50];
  memset(meta_file_name, 0, 50);
  strcpy(meta_file_name, bd);
  strcat(meta_file_name, "/");
  strcat(meta_file_name, table_name);
  strcat(meta_file_name, ".table");
  remove(meta_file_name);

  char* index_file_name = new char[50];
  memset(index_file_name, 0, 50);
  strcpy(index_file_name, bd);
  strcat(index_file_name, "/");
  strcat(index_file_name, table_name);
